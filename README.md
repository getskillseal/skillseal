...
Approval stops being a name and becomes a hash. A changed tool description is, by definition, a **different address the agent never approved** — so tampering isn't something you detect after the fact, it's something the agent can no longer express.
...
| **3. Signed, verifiable fleet state** | Fetch the trust store's **ed25519-signed namespace root**; verify it locally; flip one nibble to prove the check is real. | One request proves exactly which tool/skill versions the fleet trusts. |
...
```bash
git clone https://github.com/humuhumu33/pin-the-protocol
cd pin-the-protocol
./demo.sh
```
...
The trust store is [`UOR-Foundation/kappa-registry`](https://github.com/UOR-Foundation/kappa-registry), an OCI-style `/v2/` registry used off the shelf: it **verifies content on write** (a blob whose bytes don't match its address is rejected with `DIGEST_INVALID`) and signs a deterministic **namespace root** over every pin. `skill-lock` applies the identical approve/verify flow to `SKILL.md` files.

## Works with the agents you already use

The gateway speaks plain MCP over stdio, and `skill-lock` is a one-line pre-activation check — so this drops into any MCP- or Skills-capable agent without code changes. Point the agent's MCP config at the gateway instead of the server; wrap skill activation with `skill-lock verify`. Copy-paste configs for Claude Code, Claude Desktop, goose, Cursor, OpenClaw, Hermes, VS Code / Copilot, Gemini CLI, and others are in **[docs/compatibility.md](docs/compatibility.md)**.

```jsonc
// e.g. Claude Desktop / Claude Code mcpServers entry — wrap any server:
{
  "mcpServers": {
    "weather": {
      "command": "node",
...
| AAIF working group / roadmap item | What this demo shows |
| --- | --- |
| [Security & Privacy](https://aaif.io/) — security-by-design, adversarial testing | The tool-poisoning attack is reproduced, then made unexpressible. |
| [Identity & Trust](https://aaif.io/) — delegation you can rely on | Approval is bound to content, not a mutable name. |
| [Observability & Traceability](https://aaif.io/) — audit capabilities | A signed namespace root is a one-request, verifiable audit of fleet state. |
...
- This protects the **manifest** an agent reads. A server that behaves maliciously *without changing its advertised tools/skills* is a different problem (runtime behavior, not supply chain).
...
- The "agent" in Acts 1–2 is a deliberately naive, model-free stand-in that follows instructions found in tool descriptions — exactly the behavior that makes the attack real — so the demo is deterministic and safe to run in CI.

## Repository map

| Path | Responsibility |
| --- | --- |
| `gateway/gateway.mjs` | The verifying MCP gateway (approve + enforce). |
...
| `scripts/trust-store.sh` | Start/stop the content-addressed trust store. |
