...
// Raw-key operations, for objects that are not themselves content addressed
// (e.g. an attestation stored beside a skill).
export async function s3PutKey(key, bytes) {
  const res = await signedRequest("PUT", key, Buffer.from(bytes));
  if (!res.ok) throw new Error(`S3 put failed (${res.status})`);
}
export async function s3GetKey(key) {
  const res = await signedRequest("GET", key);
  if (res.status === 404) return null;
  if (!res.ok) throw new Error(`S3 get failed (${res.status})`);
  return Buffer.from(await res.arrayBuffer());
}
export function attestKey(address) {
  const [axis, hex] = address.split(":");
  return `attest/${axis}/${hex}.json`;
}
