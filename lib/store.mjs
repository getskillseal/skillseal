...
//
// `expectedKeyHex` is REQUIRED for a real authenticity check: it is the store
// public key captured out-of-band at approval time. Without pinning the key,
// a verifier only proves the statement is self-consistent, not that the
// trusted store produced it -- an attacker-controlled store would present its
// own key and a valid self-signature. Pass the pinned key and a substituted
// key is rejected before any signature math runs.
export function verifySignedRoot(sr, expectedKeyHex) {
  if (sr.algorithm !== "ed25519") throw new Error(`unexpected algorithm: ${sr.algorithm}`);
  if (expectedKeyHex && sr.public_key !== expectedKeyHex) {
    return false; // key substitution: not the store we approved
  }
